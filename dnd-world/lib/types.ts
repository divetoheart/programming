...
export type Milestone = {
  ...
  title: string;
  description: string;
  status: "hidden" | "active" | "complete" | "failed";
  progress: number;
  target: number;
  reward: string;
};
...
export type Rumor = {
  ...
  text: string;
  source: string;
  truth: "unknown" | "true" | "false" | "partial";
...
export type RollResult = {
  ...
  total: number;
  dc: number;
  outcome: "critical-failure" | "failure" | "success" | "critical-success";
  reason: string;
};
...
export type StoryLog = {
  id: string;
  turn: number;
  kind: "narration" | "mechanic" | "system";
  title: string;
  body: string;
  location: string;
  image?: string;
